...
import { useNextInvoiceNo } from '@/hooks/useInvoices';
...
export default function InvoiceForm({ isEdit, initialData, onSubmit, onCancel }: InvoiceFormProps) {
  ...
  useEffect(() => {
    if (isEdit && initialData) {
      setFormData({
        ...initialData,
        date: new Date(initialData.date ?? new Date()).toISOString().split('T')[0],
        dueDate: initialData.dueDate ? new Date(initialData.dueDate).toISOString().split('T')[0] : '',
      });
    }
  }, [isEdit, initialData]);

  // Set invoice number and defaults for new mode
  useEffect(() => {
    if (!isEdit && nextInvoiceNo && !formData.invoiceNo) {
      setFormData(prev => ({
        ...prev,
  ...
  useEffect(() => {
    if (formData.invoiceNo) setHasChanges(true);
  }, [formData]);
    ...
    <form onSubmit={handleSubmit} onKeyDown={handleKeyDown} className="space-y-6 max-w-5xl mx-auto pb-12">
      <div className="sm:flex sm:items-center sm:justify-between">
        ...
        <div className="mt-4 sm:mt-0 flex space-x-3 items-center">
          ...
          <button type="button" onClick={onCancel} className="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Batal</button>
